...
int PushBackPerson(PersonArr *persons, Person *new_person) {
    ...
    if (copy_worker(&(persons->arr[persons->curr_idx]), new_person) == 0) {
        persons->curr_idx += 1;
        return 0;
    } else return 1;
}
...
int PushBackTitle(TitleArr **titles, char *new_title, Person *new_person) {
    ...
    for (int i = 0; i < v->curr_idx; ++i) {
        if (strcmp(v->arr[i].title, new_title) == 0) {
            if (PushBackPerson((PersonArr *) &(v->arr[i].persons->arr), new_person) == 0) {
    ...
    if (PushBackPerson(v->arr[last_el].persons, new_person) == 0) {
        return 0;
    } else return 1;
}
